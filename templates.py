...
class EntitiesDatabase(BaseModel):
    accomodations: bool = Field(description="user asks for accomodations")
    beaches: bool = Field(description="user asks for beaches")
    activities: bool = Field(description="user asks for activities")
    gardens: bool = Field(description="user asks for gardens, parks, forests")
    thematicparks: bool = Field(description="user asks for thematic parks")
    zoos: bool = Field(description="user asks for zoos, aquariums")
    casinos: bool = Field(description="user asks for casinos")
    museums: bool = Field(description="user asks for museums, monuments, touristic sites")
...
class ItineraryTemplate(object):
    ...
    """
        ...
        self.entities_template = """
Here is some information to help you make the itinerary:
...
{entities}"""
        ...
        self.system_entities_prompt = SystemMessagePromptTemplate.from_template(
            self.entities_template, input_variables=["entities"]
        )

        self.chat_prompt = ChatPromptTemplate.from_messages(
            [self.system_message_prompt, self.human_message_prompt, self.system_entities_prompt]
        )
...
class EntitiesTemplate(object):
    def __init__(self):
        self.system_template = \
"""
You are an agent who detects the presence of certain entities in user requests.
The user's request will be denoted by four hashtags.
Determine which entities are present in the user's request.
The variable of each entity group followed by their entities are the following:
- var1: any type of accomodation
- var2: beaches
- var3: activities
- var4: gardens, parks and forests
- var5: thematic parks
- var6: zoos or aquariums
- var7: casinos
- var8: museums, monuments, and touristic sites

For each group of entities above, if the user's request mentions any of the entities, set the respective variable to True, otherwise, set the variable to False.
For example:

####
Give me an itinerary in Braga. Also suggest hotels, monuments, and aquariums that we can go to.
###

In this example, you set var1 = True, var6 = True, and var8 = True. The other entities you set to False, because the user didn't request them.
...
"""
        ...
        self.parser = PydanticOutputParser(pydantic_object=EntitiesDatabase)
